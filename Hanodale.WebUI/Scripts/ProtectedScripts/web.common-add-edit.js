        ...
        var init = function () {
            ...
            $(".multipleSelect").on("change", function () {
                ...
                if ($('#isRestBtnCalled').length > 0)
                    return false;
                $('select[data_parent_name="' + _elementId + '"]').each(function () {
                    var _id = $(this).attr('id');
                    var _actionUR = $(this).attr('data_retrieve_url');
                    if (_actionUR) {
                        $('#' + _id + ' option[value!=""]').remove();
                        $.ajax({
                            type: "POST",
                            datatype: 'json',
                            url: _actionUR,
                            data: JSON.stringify({ id: _selectedValue }),
            ...
            $('.datepicker').datepicker({
                ...
                onClose: function () {
                    ...
                    if (($('#' + id + "_Time").val() == null || $('#' + id + "_Time").val() == '') && $('#' + id + "_Time").length > 0) {
                        if ($(this).hasClass('datepicker24Format')) { $('#' + id + "_Time").val('16:00') } else { if ($(this).hasClass('datepicker24Format')) { $('#' + id + "_Time").val('16:00') } else { $('#' + id + "_Time").val('04:00 PM') } }
                    }
            ...
            $('.timepicker:not(.timepicker24Format)').timepicker({
                ...
                if (e.time.value == '') {
                    $(this).timepicker('setTime', '4:00 PM')
                }
            });
            ...
            $("#validation-form, #validation-form1").submit(function (e) {
                ...
                //var settings = $.data($('form#validation-form')[0], 'validator').settings;
                //settings.ignore = ".ignore, :disabled , :hidden";
                //$(formName).data("validator").settings.ignore = ".ignore, :disabled , :hidden";
                ////for reset the validation form
                //$(formName).validate().resetForm();


                //var validator = $(formName).data('validator');
                $('input[type="file"]').removeClass('ignore');

                $('input[type="file"]').prop('readonly', false);

                $('input[type="file"]').each(function (index, value) {
                    var fileTypeItem = $(this).attr('name');
                    if (fileTypeItem) {
                        if ($('input[name="' + fileTypeItem + '"]').length > 1) {
                ...
                if ($('#selectedFiles a.uplodedFileItem').length >= 1) {
                    //$('[name="fileUpload"]').addClass('ignore')
                    $('[name="fileUpload"]').prop('readonly', true);
                }

                $('input.ignore,textarea.ignore,select.ignore ').removeClass('input-validation-error');
                ...
                if ($(formName).valid()) {
                    ...
                    if (btnStatus == 'submit') {
                        if ($('#isSubmitAct').length > 0) {
                            $('#isSubmitAct').val(true);
                        }
                    } else {
                        if ($('#isSubmitAct').length > 0) {
                            $('#isSubmitAct').val(false);
                        }
                    }

                    if ($('#isSubmitAct').val() == "True" || $('#isSubmitAct').val() == true || $('#isSubmitAct').val() == "true") {
                        message = $("#MSG_CONFIRM_SUBMIT").val();
                    }

                    var formObj = $(this);
                    var formURL = formObj.attr("action");
                    var formData = new FormData(this);
                    $('input[data-type="currency"] , input[data-type="number"]').each(function (index, value) {
                        var $name = $(this).attr('name');
                        var purpose = $(this).val();
                        if (purpose != null && purpose != '', purpose != 'undefined') {
                            formData.set($name, purpose.replace(/,/g, ""));
                        }
                    })
                    $('input.removePerc').each(function (index, value) {
                        var $elemName = $(this).attr('name');
                        var $elemValue = $(this).val();
                        if ($elemValue != null && $elemValue != '', $elemValue != 'undefined') {
                            //if (formData.has($elemName))
                            //    formData.delete($elemName);
                            formData.set($elemName, $elemValue.replace(/%/g, "").replace(/,/g, ""));
                        }
                    })

                    navigate.formatDatetimeForSubmit(formData);
                    navigate.submitFormWithCallBack(e, formURL, formData, formObj, message, false, saveCallBack);
                    e.preventDefault(); //Prevent Default action. 
                    //e.unbind();
                } else {
                    setTimeout(function () {
                        var input = $('.input-validation-error:first');
                        if (input) { input.focus(); scrollToElement(input); }
                    }, 100);
                    $('input[type="file"]').prop('readonly', false);
         ...
         saveCallBack = function (data) {
             ...
             if (data.status == "Success") {
                 ...
                 if (data.refreshDatatable) {
                     $('#dt_' + partName).dataTable()._fnAjaxUpdate();
                 }
             }
         },

         saveCallBack1 = function (data) {
             //message.show(data.status, data.message);
             if (data.status == "Success") {
