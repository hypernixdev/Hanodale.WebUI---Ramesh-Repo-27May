...
function drawBarChart(controllerName, actionName, isCategortByName) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ startDate: fromDate, endDate: toDate, type: $("#sectionType_Id").val() }),
        cache: false,
        hideLoading: true,
        ...
        success: function (data) {
            for (var k = 0; k < data.list.length; k++) {
                ...
                var entity = obj.lstItem;
                var subList = [];
                if (isCategortByName) {
                    for (var f = 0; f < entity.length; f++) {
                        var index = 0;
                        var labelName = entity[f].categoryName
                ...
                status.push({ data: subList, label: dataList[k].categoryName });
                ...
                console.log(subList)
                console.log(yaxis)
                $.plot($("#barCharPanelDashboardPlaceholderBody_" + obj.sectionType), subList,
                    {
                        xaxis: xaxis,
                        yaxis: yaxis,
                        series: {
                        ...
                        legend: {
                            show: true,
                            container: $("#barChartLegend_" + obj.sectionType),
                            noColumns: 3,
                            flexibleLegend: true
                        },
                        zoom: {
                ...
                $("#barCharPanelDashboardPlaceholderBody_" + obj.sectionType).UseTooltip();
                //$("#barCharPanelDashboardPlaceholderBody_" + obj.sectionType).resizable({
                //    maxWidth: 900,
                //    maxHeight: 500,
                //    minWidth: 270,
                //    minHeight: 250
